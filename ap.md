...
If this directive is present, the Patcher MUST stop processing and ignore any text that follows it. The AI SHOULD append an `[ID] END` directive to mark the end of the patch safely.

## 3. Patcher Implementation Requirements

### 3.0. Atomicity

When operating in Strict Mode, a Patcher MUST treat the application of an entire patch file as a single, atomic transaction. If any modification cannot be successfully applied, the Patcher MUST abort the entire operation and MUST NOT write any changes to any files on the filesystem.

In Tolerant Mode (RECOMMENDED by default), the Patcher MAY relax atomicity to apply successfully resolved modifications and skip failing ones, logging the failures to a separate file (e.g., `afailed.ap`).

### 3.1. Idempotency

A Patcher MUST apply modifications idempotently. Applying the same patch multiple times to the same source tree MUST NOT cause subsequent changes or errors after the first successful application. When a modification is skipped due to idempotency, the Patcher MUST NOT fail, but SHOULD output a visible warning (e.g., `~ SKIPPED (Idempotency): Looks like it's already applied`). To achieve this, each modification MUST be checked for its state before the operation is attempted, according to the following rules:
...
The Patcher MUST remove all trailing whitespace characters (spaces and tabs) from the newly inserted `content` blocks. To preserve semantic integrity and avoid polluting version control history, the Patcher MUST NOT automatically remove trailing whitespace from unmodified portions of the target file.
