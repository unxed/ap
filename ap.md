...
- `snippet`: A string of code to locate. If `end_snippet` is also provided, this marks the beginning of the range.
...
- `end_snippet`: An OPTIONAL string of code that marks the end of a range. It MUST be used together with `snippet`.
...
**Range-based Search**: If both `snippet` and `end_snippet` are provided (for `REPLACE` or `DELETE` actions):
...
2.  After finding the `snippet`, the Patcher searches for the *first* occurrence of the `end_snippet` that appears *after* the end of the `snippet`.
3.  If the `snippet` is found but the `end_snippet` is not found in the remainder of the search scope, the Patcher MUST report an error. The entire region from the beginning of the `snippet` to the end of the `end_snippet` is considered the target for the modification.
...
-   **Range Auto-Correction:** When processing a `REPLACE` or `DELETE` action with a `snippet`/`end_snippet` pair, the Patcher SHOULD first check if the `end_snippet` is a suffix of the `snippet`. If it is, the Patcher SHOULD treat this as a single-snippet operation, use the `snippet` for it and ignore the `end_snippet`. This corrects a frequent AI error where the entire block to be replaced is put into `snippet`, causing a "end snippet not found" failure.
...
2.  **Choose the Locator Type Based on the Change**:
    -   For a **range change** (`REPLACE` or `DELETE`), the AI SHOULD use the `snippet`/`end_snippet` pair.
...
3.  **Select the Snippet(s) and Anchor (if needed)**: After choosing the locator type, the AI proceeds to select the content for the fields:
    ...
    -   **For a `snippet`/`end_snippet` pair**:
...
- **Choose Stable and Independent Snippets for Ranges:** When using a `snippet`/`end_snippet` pair, the AI MUST adhere to the following critical rules to avoid generating failing patches:
    1.  **Snippets MUST be independent.** The `end_snippet` MUST be located entirely *after* the `snippet` in the original file. They cannot overlap, and the `end_snippet` cannot be a part of the `snippet`.
    2.  **Snippets MUST be minimal.** The `snippet` and `end_snippet` should be as short as possible while uniquely identifying the start and end of the desired block. Do NOT include the entire content of the block to be replaced inside the `snippet`.
