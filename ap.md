...
2.  **Redundancy is allowed:** If an AI provides both a single line `snippet` and a block `start_snippet` for the same change, the tool should accept it and figure out the intent, rather than throwing a validation error.
3.  **Flexibility is Key:** If an AI forgets to explicitly declare a `FILE` before a `CREATE_FILE` action, the tool should infer the intent from the context.
## 1. Introduction
...
-   **Locator Precedence (The "Hallucination" Fix):** AI models frequently "hallucinate" extra fields, providing both a single `snippet` and a `start_snippet`/`end_snippet` range for the same action. The Patcher MUST prioritize the range (`start_snippet`/`end_snippet`) and ignore the single `snippet` if both are present.
-   **Locality Heuristic:** AI models almost always modify files sequentially. If an `anchor` or `snippet` matches multiple locations (ambiguity), the Patcher SHOULD prioritize the match that appears *after* the end of the previously applied modification in the same file.
-   **Implicit File Creation:** If a `CREATE_FILE` directive is followed by a value (a file path) instead of being empty, the Patcher SHOULD treat this as an implicit declaration of the target file, equivalent to a `FILE` directive followed by `CREATE_FILE`.
